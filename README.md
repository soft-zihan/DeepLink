...
1. 目前在 v1.3 版本，新版本会添加分组颜色修改。当然版本仍然存在bug和明显的不完善。除了继续完善壁纸添加、修复相机等功能，后续预计：
   1.1 使用图标代替链接名称，将编辑功能分离主页面。优化UI设计（当前比较丑）。
   1.2 增加更多预设链接，比如网盘搜索、开发人员搜索等。微信公众号、贴吧暂未反编译成功。
   1.3 有更远的计划引入AI搜索功能，搜集汇总所选平台的搜索结果。
2. 可能开发用于PC的web端，在同一个界面聚合所有搜索结果或其他链接（比如全平台收藏），但对比rss存在性能问题，也许会因为鸡肋而放弃。
