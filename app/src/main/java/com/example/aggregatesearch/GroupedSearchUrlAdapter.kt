...
class GroupedSearchUrlAdapter(
    ...
    override fun onAttachedToRecyclerView(recyclerView: RecyclerView) {
        ...
        recyclerView.doOnLayout {
            // 修复：不再自动清除 pendingStateChanges，因为这会导致图标状态在刷新后被重置
            // 只记录日志，待处理状态由具体的刷新操作完成后清理
            android.util.Log.d("DeepLink", "RecyclerView 布局完成")
        }
    }
        ...
        if (payloads.isNotEmpty()) {
            when (holder) {
                is IconsContainerViewHolder -> {
                    ...
                    if (payload == "REFRESH_ICON_STATES") {
                        // 强制刷新图标状态，忽略待处理状态
                        val item = getItem(position) as SearchUrl
                        val urls = getUrlsForGroup(item.groupId)
                        if (urls.isNotEmpty()) {
                            holder.refreshIconStates(urls)
                        }
                        return
                    }
                }
    ...
    /**
     * 强制刷新所有图标的选中状态 - 用于全选操作后的UI同步
     */
    fun forceRefreshAllIconStates() {
        // 清空所有待处理状态，以当前数据库状态为准
        pendingStateChanges.clear()
        ...
        for (i in currentItems.indices) {
            ...
            if (item is SearchUrl) {
                // 对于图标容器类型的项目，使用payload刷新
                val urls = getUrlsForGroup(item.groupId)
                if (urls.isNotEmpty() && urls.first().id == item.id) {
                    // 这是图标容器的第一个URL，刷新整个容器
                    notifyItemChanged(i, "REFRESH_ICON_STATES")
                }
            }
        }
    }
    ...
    fun updateGroupCheckedState(group: UrlGroup, isChecked: Boolean) {
        ...
        // 同时刷新该分组的所有图标状态
        refreshGroupIconStates(group.id)
    }

    /**
     * 刷新指定分组的图标状态
     */
    private fun refreshGroupIconStates(groupId: Long) {
        // 清除该分组所有URL的待处理状态
        val urlsInGroup = getUrlsForGroup(groupId)
        urlsInGroup.forEach { url ->
            pendingStateChanges.remove(url.id)
        }

        // 找到该分组的图标容器并刷新
        val containerPosition = currentList.indexOfFirst { item ->
            item is SearchUrl && item.groupId == groupId &&
            getUrlsForGroup(item.groupId).firstOrNull()?.id == item.id
        }

        if (containerPosition != -1) {
            notifyItemChanged(containerPosition, "REFRESH_ICON_STATES")
        }
    }
    ...
    inner class GroupViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private fun applyDefaultBackground(view: View) {
            ...
            try {
                // 修复R.attr引用，使用com.google.android.material.R.attr代替
    ...
    inner class IconsContainerViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        // 新增方法：强制刷新图标状态
        fun refreshIconStates(urls: List<SearchUrl>) {
            // 清空现有视图
            iconsContainer.removeAllViews()
            ...
            urls.forEach { searchUrl ->
                ...
                // 确定当前图标的状态
                val isEnabled = pendingStateChanges[searchUrl.id] ?: searchUrl.isEnabled
                android.util.Log.d("DeepLink", "重新绑定图标 ${searchUrl.name}. 待定状态: ${pendingStateChanges[searchUrl.id]}, 原始状态: ${searchUrl.isEnabled}, 最终状态: $isEnabled")
